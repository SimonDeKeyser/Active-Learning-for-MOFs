...
def wrap_model(scaled_model: RescaleOutput) -> RescaleOutput:
    ...
    gradient_attack_model = GradientOutput( func= gradient_model.func, 
                                            of= gradient_model.of,
                                            wrt= gradient_model.wrt,
                                            out_field= gradient_model.out_field,
                                            sign= gradient_model.sign
    )

    scaled_attack_model = RescaleOutput(model=gradient_attack_model, 
    ...
    if scaled_attack_model.shift_by.nelement() == 0:
        scaled_attack_model.has_shift = None 

    if scaled_attack_model.scale_by.nelement() == 0:
        scaled_attack_model.has_scale = None 
                                        
    return scaled_attack_model